...
void CreateCards()
{
	...
	// 6 Weapon Cards
	baraja.push_back(carta(WEAPON, "Pu�al", 7));
...
void TCPProtocol(Network::TCP::Server &server, sf::Packet& packet, int roomIndex, int socketIndex) {
	...
	case START:
		...
		if (roomIndex == -1 && TCP_SERVER.RoomsCount() == 0) { //El jugador esta a la llista d'espera i no hi ha sales creades
			...
			for (int i = 0; i < TCP_SERVER.RoomsCount(); i++) {
				if (TCP_SERVER.GetRoom(i).GetSocketsCount() < TCP_SERVER.GetRoom(i).MaxClients()) { //Hi ha espai a la sala
					...
					if (TCP_SERVER.GetRoom(i).GetSocketsCount() == TCP_SERVER.GetRoom(i).MaxClients()) { // L'habitaci� ja est� plena, enviem el START de partida als clients que hi ha dins
						...
						for (int j = 0; j < TCP_SERVER.GetRoom(i).GetSocketsCount(); j++) {
							...
							for (std::list<carta>::iterator it = players[i + j].myCards.begin(); it != players[i + j].myCards.end(); ++it) //ID de les cartes que t� el client
							{
								startPacket << it->id;
							}
						...
						TirarDaus(i,0); //Tirem els primers daus de la partida tot assignant qui ser� el jugador que comen�ar�
					}
					break;
				}
			}
		...
		break;
		//Enviar als clients que comen�a la partida
